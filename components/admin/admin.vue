<template>
  <div>
    <div v-if="isContentShow" class="blue pa-2">
      <p class="text-center">Admin Ne yapmak istersin</p>
    ...
    <div v-else>
        ...
        <v-card color="d-flex justify-center">
          <div style="width: 50%">
            <div v-for="(item, index) in accadion">
              <v-btn @click="updateBtnWoeld(item, index)">{{
                item.kelime
              }}</v-btn>
            </div>
          </div>
      ...
      <v-card v-else class="card">
        ...
        <template>
          ...
          <v-card
            v-if="world.exampleSentences.length > 0"
            class="pa-2 card-content"
          >
            <p class="text-center">Anlamı</p>
            <v-row class="">
              <v-col
                cols="12"
                v-for="(tag, index) in world.exampleSentences"
                class="white--text px-4 py-2 rounded-lg mt-2 mb-2"
                :key="index"
              >
                <v-card
        ...
        <template>
          ...
          >
            ...
            <v-row>
              <v-col
                cols="12"
                v-for="(tag, index) in world.meaning"
                class="px-4 py-2 rounded-lg mt-2 mb-2"
                :key="index"
              >
                <v-card
                  style="width: 100%; border: 1px solid black"
                  class="d-flex pa-2 black--text"
                >
                  <v-btn
                    @click="deleteCumleTag(tag)"
                    x-small
                    fab
                    icon
                    class="blue white--text mr-2"
  ...
  methods: {
    send() {
      if (this.categories != null) {
        ...
        this.$axios
          .post(
            "https://englishworld-db088-default-rtdb.europe-west1.firebasedatabase.app/categories/" +
              uid +
              "/" +
              this.categories +
              ".json",
          ...
          .then((res) => {
            ...
            this.world.meaning = [];
            (this.anlamnewTag = null), (this.cumlenewTag = null);
          });
      }
    },
    updatePage() {
    ...
    deleteTag(tag) {
      this.world.exampleSentences.splice(
        this.world.exampleSentences.indexOf(tag),
        1
      );
    },
